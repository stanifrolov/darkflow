	...
	else
	{
		...
		for (int i = 0; i < numPoints1; i++)
		{
			...
			else
			{
				...
				double x_overlap = max(0, min(l1[i] + w1[i], l2[position[i]] + w2[position[i]]) - max(l1[i], l2[position[i]]));
				double y_overlap = max(0, min(t1[i] + h1[i], t2[position[i]] + h2[position[i]]) - max(t1[i], t2[position[i]]));
...
void compute(double* tr1, double* tr2, const int* dim1, const int* dim2, double threshold, bool world, double& cost, double& fp, double& fn)
{
	...
	tr1start = tr1[index(1,2, numPoints1)];
	tr1end = tr1[index(numPoints1,2, numPoints1)];
	tr2start = tr2[index(1,2, numPoints2)];
	tr2end = tr2[index(numPoints2,2, numPoints2)];
	...
	double* frames1 = &tr1[index(1, 2, numPoints1)];
	double* frames2 = &tr2[index(1, 2, numPoints2)];
